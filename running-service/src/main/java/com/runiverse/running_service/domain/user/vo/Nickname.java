...
public record Nickname(String value) {
    ...
    public Nickname {
        if (value == null) throw new NicknameRequiredException();
        value = value.trim();
        if (value.isEmpty()) throw new NicknameRequiredException();
        if (value.length() < MIN_LENGTH || value.length() > MAX_LENGTH)
            throw new InvalidNicknameLengthException();
        if (!PATTERN.matcher(value).matches()) throw new InvalidNicknameFormatException();
    }
}
