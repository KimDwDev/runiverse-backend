...
import com.runiverse.running_service.infrastructure.persistence.common.BaseCreatedAtJpaEntity;
...
public class BaseTimeJpaEntityTest {
    ...
    void createdAtHasCreationTimestamp() throws NoSuchFieldException {
        ...
        Field createdAt = BaseCreatedAtJpaEntity.class.getDeclaredField("createdAt");
    ...
    void updatedAtHasUpdateTimestamp() throws NoSuchFieldException {
        // when & then
        Field updatedAt = BaseTimeJpaEntity.class.getDeclaredField("updatedAt");
    ...
    @Test
    @DisplayName("BaseTimeJpaEntity는 생성 시각도 함께 상속한다")
    void baseTimeEntityInheritsCreatedAt() {
        // when & then -> 수정 시각만 있는 테이블은 없다
        assertThat(BaseTimeJpaEntity.class.getSuperclass()).isEqualTo(BaseCreatedAtJpaEntity.class);
    }
    ...
    void bothAreMappedSuperclass() {
        ...
        assertThat(BaseCreatedAtJpaEntity.class.isAnnotationPresent(MappedSuperclass.class)).isTrue();
        assertThat(BaseTimeJpaEntity.class.isAnnotationPresent(MappedSuperclass.class)).isTrue();
    }
}
