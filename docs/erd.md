...
- **타임스탬프**: `*_at`은 전부 `timestamptz`(UTC 저장, API `Z` 표기). `*_date`도 시점이면 `timestamptz`. **예외로 달력 날짜**(대회 `event_date`·`registration_start_date`·`registration_end_date`, `user_onboardings.birthday`)는 `date`(시각·시간대 없음, API `YYYY-MM-DD`).
