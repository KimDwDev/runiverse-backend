...
논리적 변경을 잘게 잡는다. 기능 하나가 여러 커밋으로 나뉘는 것이 정상이다.

- VO·포트처럼 독립적으로 성립하는 구성 요소는 **하나당 커밋 하나**로 나눈다.
- 테스트는 기능 커밋에 섞지 않고 **별도 `✅ Test` 커밋**으로 낸다. 레이어 묶음이 끝나는 시점에 몰아서 작성한다.
...
- 제목에는 이슈 키를 앞에 붙이고 이모지는 쓰지 않는다. 본문 첫 줄(`# 📌 …`)에는 템플릿의 이모지를 유지한다.
