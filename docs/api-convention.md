...
- ID 타입: `userId`만 UUID, 그 외 **리소스 ID**(`feedId`·`commentId`·`contestId`·`runningRecordId`·`runningSessionId`·`colorId` 등 서버가 발급하는 PK)는 Long. 클라이언트가 만드는 식별자(`deviceId`)와 커서(`cursor`)·S3 key는 문자열.
