...
- 표 머리는 `필드 | 타입 | 필수 | 설명`로 통일한다. `타입`에는 JSON 타입만 적고(`String`·`Integer`·`Number[][]`·`Array`·`Boolean`), `... 또는 null` 같은 표기는 쓰지 않는다 — 그건 `필수` 열과 설명이 맡는다.
- **`필수` 열은 요청과 응답에서 뜻이 다르다.** 요청은 "클라가 반드시 보내야 하는가"(`X`면 생략 가능), 응답은 "값이 반드시 있는가"(`X`면 `null`일 수 있음)다.
- **응답의 필드 키는 값이 `null`이어도 항상 내려간다** — 키가 빠지는 경우는 없다(Jackson 기본 직렬화). `필수 X`를 "키가 없을 수 있다"로 읽지 않는다.
- `필수 X`인 응답 필드는 **언제 `null`이 되는지**를 설명에 적는다.
